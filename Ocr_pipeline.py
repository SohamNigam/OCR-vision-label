...
os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = "path/to/your/credentials.json"  # Update with your credentials path
...
def list_pdfs_in_bucket_folder(bucket_name: str, folder_prefix: str) -> List[str]:
    ...
    for blob in blobs:
        if blob.name.lower().endswith('.pdf'):
            uri = f"gns://{bucket_name}/{blob.name}"
            pdf_uris.append(uri)
    return pdf_uris
...
def process_pdf(pdf_path, gemini_model, conf_map, output_folder=None):
    ...
    if output_folder:
        ...
        # print(f"DEBUG: contents of {output_folder} = {os.listdir(output_folder)}")
